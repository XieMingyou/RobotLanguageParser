...
char *varProgName; //主程序的名称
char *projName; //主程序所在项目的名称
...
int varProgCompile()
{
	while (!varProgQueue.empty())
	{
		...
		*labelHead = initialLabel;
		...
		char *varFilePath = new char[strlen(projName) + strlen(curVarProg) + 40];
		sprintf(varFilePath, "%s%s%s%s%s", ".\\prog_var_xml\\var\\", projName, "\\", curVarProg, ".var");
		...
		delete[]varFilePath;
		char *progFilePath = new char[strlen(projName) + strlen(curVarProg) + 40];
		sprintf(progFilePath, "%s%s%s%s%s", ".\\prog_var_xml\\prog\\", projName, "\\", curVarProg, ".prog");
